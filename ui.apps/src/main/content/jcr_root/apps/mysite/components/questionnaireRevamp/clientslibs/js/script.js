document.addEventListener("DOMContentLoaded", function () {
    ...
    function setupImageSelection(sectionId, minSelections, maxSelections) {
        ...
        function handleCheckboxChange(event) {
            const checkbox = event.target; // Get the checkbox that was clicked
            const card = checkbox.closest('.image-card'); // Get the parent card element
            const value = checkbox.value; // Get the value of the checkbox
        
            if (checkbox.checked) {
                // If the checkbox is checked
                if (selectedImages.length < maxSelections) {
                    // card.classList.add('selected'); // Add selected class to the card
                    selectedImages.push(value); // Add the value to the selected images array
                } else {
                    // Show notification and uncheck the box if max is reached
                ...
                selectedImages = selectedImages.filter(id => id !== value); // Remove value from the array
            }
        
            // Log only checked checkbox values
            const checkedValues = Array.from(
                document.querySelectorAll('input[name="ridingPosition"]:checked')
        ...
        imageGrid.addEventListener('click', handleCheckboxChange);
...
// const bgImage = $('.bgimageForFirstFiveSection');
// console.log(window.innerWidth);
function updateDisplay() {
      if ($('.section-one .main-background').hasClass('active')) {
        $('.section-one .bgimageForFirstFiveSection').css('display', 'none');
        $('.section-one .bgimageForFirstFiveSection').css('width', '0');
      }

      if(!(".questionnaire-section").hasClass("active")){
        $('.section-one .bgimageForFirstFiveSection').css('display', 'none');
        $('.section-one .bgimageForFirstFiveSection').css('width', '0');
      }
  }
