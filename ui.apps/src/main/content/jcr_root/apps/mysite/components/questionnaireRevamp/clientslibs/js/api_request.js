...
function getDataFromServlet() {
    const checkboxes = document.querySelectorAll('input[type="checkbox"][name="ridingPosition"]:checked');
    const currentSelections = new Set();

    checkboxes.forEach(checkbox => {
        const value = checkbox.value;  // Get the value of the checked checkbox
        if (value) {
            currentSelections.add(value);  // Add the value to the Set
        }
    });
