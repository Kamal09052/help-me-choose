...
class StepwiseQuestionnaireModelImplTest {
    ...
    void getAnswers() {
        ...
        List<Map<String, String>> result = stepwiseQuestionnaireModel.getAnswers();
        ...
        assertEquals("1", result.get(0).get("index"), "The index of the first answer should be '1'.");
        assertEquals("Answer 1", result.get(0).get("answers"), "The first answer should be 'Answer 1'.");

        assertEquals("2", result.get(1).get("index"), "The index of the second answer should be '2'.");
        assertEquals("Answer 2", result.get(1).get("answers"), "The second answer should be 'Answer 2'.");
    }
    ...
    void getScreen2Answers() {
        // Mock screen2 to indicate it has children
        ...
        when(valueMap1.get("pillionFrequencyImage", String.class)).thenReturn("Image1.png");

        when(childResource2.getValueMap()).thenReturn(valueMap2);
        when(valueMap2.get("checkbox", String.class)).thenReturn("Option 2");
        when(valueMap2.get("pillionFrequencyImage", String.class)).thenReturn("Image2.png");

        // Act: Call the method
        List<Map<String, String>> result = stepwiseQuestionnaireModel.getScreen2Answers();
        ...
        Map<String, String> firstAnswer = result.get(0);
        assertEquals("Option 1", firstAnswer.get("checkbox"), "The first checkbox answer should be 'Option 1'.");
        assertEquals("Image1.png", firstAnswer.get("pillionFrequencyImage"), "The first pillionFrequencyImage should be 'Image1.png'.");

        // Verify the content of the second result
        Map<String, String> secondAnswer = result.get(1);
        assertEquals("Option 2", secondAnswer.get("checkbox"), "The second checkbox answer should be 'Option 2'.");
        assertEquals("Image2.png", secondAnswer.get("pillionFrequencyImage"), "The second pillionFrequencyImage should be 'Image2.png'.");
    }
    ...
    void getRidingPositionImagesURL() {
        ...
        // Act: Call the method
        List<Map<String, String>> result = stepwiseQuestionnaireModel.getRidingPositionImagesURL();
